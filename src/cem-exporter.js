(function (root, factory) {
  ...
  function emitModelCase(model, modelId, modelScale) {
    ...
    const textureSource = model.textureSource || 'static_atlas';
    const animation = model.textureAnimation || {frameCount: 1, frameDurationTicks: 1};
    if (!['static_atlas', 'host_sampler0', 'animated_sampler0'].includes(textureSource)) throw new Error(`unsupported model texture source: ${textureSource}`);
    if (!Number.isInteger(animation.frameCount) || animation.frameCount < 1 || !Number.isInteger(animation.frameDurationTicks) || animation.frameDurationTicks < 1) throw new Error('model textureAnimation is invalid');
    const textureSetup = textureSource === 'animated_sampler0'
      ? [`    cem_texture_source = 1;`, `    cem_texture_frame_count = ${animation.frameCount};`, `    cem_texture_frame_ticks = ${animation.frameDurationTicks};`]
      : [`    cem_texture_source = 0;`, `    cem_texture_frame_count = 1;`, `    cem_texture_frame_ticks = 1;`];
    return [
      `case ${modelId}:`,
      '{',
      ...textureSetup,
  ...
  function exportModel(model, modelId = 1, cemVersion = '1.21.6', options = {}) {
    ...
    const glsl = `// Generated by CEM-S Studio for ${model.name}.\n${emitModelCase({...model, textureSource: options.textureSource || model.textureSource, textureAnimation: options.textureAnimation || model.textureAnimation}, modelId, options.modelScale ?? 8)}`;
    return {glsl, manifest: {format: 'cem-s-studio/1', model: model.name, target: `cem-s/${cemVersion}`}};
  }

  function exportModels(entries, baseModelId = 1, cemVersion = '1.21.6', options = {}) {
    ...
    const cases = entries.map(entry => {
      ...
      ids.add(modelId);
      return emitModelCase({...entry.model, textureSource: options.textureSource || entry.model.textureSource, textureAnimation: options.textureAnimation || entry.model.textureAnimation}, modelId, entry.branch?.modelScale ?? 8);
    });
    return {
